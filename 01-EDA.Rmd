...
```{r estructura-eda}
cat(sprintf("Dimensiones del dataset: %d pacientes x %d variables\n",
            nrow(df_mutations), ncol(df_mutations)))
cat(sprintf("Registros duplicados: %d\n", sum(duplicated(df_mutations))))
cat(sprintf("Valores faltantes: %d\n", sum(is.na(df_mutations))))
```
...
```{r grade-full-plot}
conteo <- df_mutations |> count(Grade) |> mutate(pct = n / sum(n) * 100)
...
ggplot(conteo, aes(x = Grade, y = n, fill = Grade)) +
  ...
  scale_x_discrete(labels = c("LGG (0)", "GBM (1)")) +
...
split_glioma <- initial_split(df_mutations, prop = 0.80, strata = Grade)

train_data <- training(split_glioma)
test_data  <- testing(split_glioma)

X_train <- train_data |> select(-Grade)
y_train <- train_data$Grade
X_test  <- test_data  |> select(-Grade)
...
```{r grade-train-donut}
conteo <- train_data |> count(Grade) |> mutate(pct = n / sum(n) * 100)
...
analisis_bivariado <- function(df, target, variable, etiquetas = NULL,
  ...
  p <- ggplot(df_plot, aes(x = categoria, y = n, fill = .data[[target]])) +
    ...
    scale_fill_manual(values = PALETTE_GRADE, labels = c("Grade 0", "Grade 1")) +
...
ggplot(df_target_corr, aes(x = variable, y = r, fill = signo)) +
  ...
  scale_fill_manual(values = c("neg" = PALETTE_GRADE["0"], "pos" = PALETTE_GRADE["1"]), guide = "none") +
...
Dado que ningún par de variables independientes superó el umbral crítico de redundancia ($V > 0,70$ o $\vert{}r\vert{} > 0,70$), se confirma la **ausencia de multicolinealidad estructural** en la muestra de entrenamiento. Este resultado valida la retención de este subconjunto de biomarcadores, permitiendo a los algoritmos de clasificación explotar estas interacciones complejas sin generar inestabilidad en la estimación de parámetros.

```{r guardar-datos-eda}
# Transferencia de datos hacia los cuadernos de modelado
saveRDS(
  list(X_train = X_train, X_test = X_test, y_train = y_train, y_test = y_test),
  file = "datos_modelado.rds"
)
```
