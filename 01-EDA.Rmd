...
```{r}
kable(tail(df_mutations), caption = "Últimos registros del conjunto de datos.") |>
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"),
                full_width = TRUE, font_size = 12) |>
  scroll_box(width = "100%")
```
...
info_df <- tibble(
  ...
  `Valores únicos`   = map_int(df_mutations, ~ n_distinct(.x))
)
...
kable(t(map_chr(df_mutations, ~ class(.x)[1])), caption = "Tipo de dato por variable tras la recodificación.") |>
...
ggplot(conteo, aes(x = Grade, y = n, fill = Grade)) +
  ...
  labs(title = "Distribución de la variable objetivo: Grado tumoral",
...
La partición 80/20 estratificada por `Grade` (semilla fija = 42) se realizó una única vez para todo el proyecto y se exportó a `train_data.csv` / `test_data.csv`, de modo que todos los cuadernos —tanto los de EDA como los de modelado, en Python o en R— parten exactamente de la misma partición y no existe riesgo de que cada entorno genere una división distinta. Por ello, aquí simplemente se leen esos archivos en lugar de recalcular la partición.

```{r split-eda}
train_data <- read_csv("Dataset/train_data.csv", show_col_types = FALSE)

# Se recodifican las variables categóricas exactamente igual que en df_mutations,
# para que el resto del documento (detección automática de variables genéticas,
# pruebas de independencia, paleta de colores, etc.) funcione sin cambios.
train_data <- train_data |> mutate(across(all_of(columnas_categoricas), as.factor))
...
summary_age <- tibble(
  ...
  Valor = c(
    ...
    sprintf("%.4f", kurtosis(age_vec) - 3)  # curtosis en exceso, análoga a pandas .kurt()
  )
)
...
p_hist <- ggplot(train_data, aes(x = Age_at_diagnosis)) +
  ...
  labs(title = "Distribución de Edad al Diagnóstico (Train Set)",
...
p_box <- ggplot(train_data, aes(x = Age_at_diagnosis)) +
  ...
  labs(title = "Boxplot de Edad (Detección de Outliers)", x = "Edad (Años)") +
...
outliers <- train_data |> filter(Age_at_diagnosis < limite_inferior | Age_at_diagnosis > limite_superior)

cat("EVALUACIÓN MATEMÁTICA DE OUTLIERS (MÉTODO IQR)\n")
cat(sprintf("• Límite Inferior Aceptable: %.2f años\n", limite_inferior))
cat(sprintf("• Límite Superior Aceptable: %.2f años\n", limite_superior))
cat(sprintf("• Cantidad de outliers detectados: %d\n", nrow(outliers)))
```

```{r age-normalidad-eda, fig.width=6, fig.height=4.5}
shapiro_res <- shapiro.test(age_vec)

cat("EVALUACIÓN DE NORMALIDAD: EDAD AL DIAGNÓSTICO (TRAIN SET)\n")
cat(sprintf("• Asimetría (Skewness): %.4f\n", skewness(age_vec)))
cat(sprintf("• Curtosis: %.4f\n", kurtosis(age_vec) - 3))
cat(sprintf("• Test Shapiro-Wilk: Estadístico W = %.4f\n", shapiro_res$statistic))
...
kable(tabla_gender, caption = "Frecuencia por género (Train Set).") |>
  kable_styling(bootstrap_options = c("striped", "condensed"), full_width = FALSE)
```
...
ggplot(tabla_race, aes(x = `Grupo racial`, y = Frecuencia)) +
  ...
  scale_y_continuous(expand = expansion(mult = c(0, 0.15))) +
  labs(
    title = "Distribución por grupo racial",
    x = "Grupo racial",
    y = "Frecuencia"
  )
...
plots_genes_univ <- map(variables_geneticas, function(v) {
  tabla <- train_data |> count(.data[[v]]) |> mutate(pct = n / sum(n) * 100)
  ggplot(tabla, aes(x = .data[[v]], y = n)) +
    geom_col(fill = "#4C72B0", width = 0.5) +
    geom_text(aes(label = n), vjust = -0.4, size = 3) +
    labs(title = v, x = "Categoría", y = "Frecuencia") +
